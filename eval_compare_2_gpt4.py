...
def drawevalplot(scores):
    ...
    colors = ["#802A2A", "#B22222", "#B22222"]
    
    #误差线使用标准偏差SD 方差的平方根
    models = ["GPT4", "llama7b", "llama13b"]
...
if __name__ == '__main__':
    ...
    with open("modelanswers/LLaMA7B_test_basic.txt", "r", encoding="utf-8") as f:
        ll7b = f.readlines()
    f.close()
    
    with open("modelanswers/LLaMA13B_test_basic.txt", "r", encoding="utf-8") as f:
        ll13b = f.readlines()
    f.close()

    with open("modelanswers/gpt4_test_basic.txt", "r", encoding="utf-8") as f:
        gpt4 = f.readlines()
    f.close()

    with open("modelanswers/groundtruth.txt", "r", encoding="utf-8") as f:
        answer = f.readlines()
    f.close()
    
    model_answers = {"GPT4":gpt4, "LLaMA7B":ll7b, "LLaMA13B":ll13b}
    ...
    models = ["GPT4", "LLaMA7B", "LLaMA13B"]
    scores = {"GPT4":[], "LLaMA7B":[], "LLaMA13B":[]}
    
    print("开始打分")
    
    for j in range(exp_times):
        reviews = {"GPT4":[], "LLaMA7B":[], "LLaMA13B":[]}
        review_scores = {"GPT4":[], "LLaMA7B":[], "LLaMA13B":[]}
        for model in models:
        ...
        for i in range(len(reviews["GPT4"])):
            review.append([reviews["GPT4"][i], reviews["LLaMA7B"][i], reviews["LLaMA13B"][i]])
        review = pd.DataFrame(columns=["GPT4回答评价", "LLaMA7B回答评价", "LLaMA13B回答评价"], data=review)
        review.to_csv("reviews/magua_base_generation_test_review_llama_"+ str(j+1) +".csv", encoding="utf-8", header=1, index=1)
            
        # 计s算每个模型的平均分
        for model in models:
            temp = np.array(review_cores[model])
